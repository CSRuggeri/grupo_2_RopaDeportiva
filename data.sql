...
CREATE TABLE IF NOT EXISTS `sportGoDb`.`category` (
  ...
  INDEX `fk_Products_category1_idx` (`category_id` ASC) VISIBLE,
    ...
    REFERENCES `sportGoDb`.`category` (`category_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
INSERT INTO product (id,name, price, stock, description, category, brand, gender, image, discount, size, category_id, brand_id)
VALUES 
